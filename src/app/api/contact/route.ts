  ...
  try {
    ...
    try {
      ...
      if (referrerId) {
        ...
        const referrerResponse = await fetch('https://api.brevo.com/v3/smtp/email', {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
        ...
        if (!referrerResponse.ok) {
          const error = await referrerResponse.json();
        ...
        const referrerEmailData = await referrerResponse.json();
        console.log('Referrer notification sent successfully:', referrerEmailData);
      }
    } catch (emailError) {
